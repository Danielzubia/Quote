...
// Note: html2canvas is loaded globally via script tag, but we're logging its presence
...
export function QuoteDialog({
  ...
  linearFeet: initialLinearFeet = 0,
  surfaceArea: initialSurfaceArea = 0,
  address,
  mode = 'spaces',
  processedImageId: initialProcessedImageId
}: QuoteDialogProps) {
  ...
  const [linearFeet, setLinearFeet] = useState<number>(initialLinearFeet || 0);
  ...
  // Initialize processedImageId when dialog opens with the ID passed from parent
  useEffect(() => {
    if (open && initialProcessedImageId) {
      console.log("Setting processed image ID from prop:", initialProcessedImageId);
      setProcessedImageId(initialProcessedImageId);
    }
  }, [open, initialProcessedImageId]);
  
  // Update linear feet when the prop changes
  useEffect(() => {
    console.log("Updating linear feet from prop:", initialLinearFeet);
    setLinearFeet(initialLinearFeet || 0);
  }, [initialLinearFeet]);
  ...
  const calculateQuote = () => {
    if (mode === 'area' && surfaceArea) {
      ...
      console.log(`Linear feet calculation: ${linearFeet} feet × $2 = $${linearFeetCost}`)
      
      // Total base estimate
      const baseEstimate = mobilizationFee + parkingCost + handicapCost + crosswalkCost + arrowsCost + linearFeetCost;
  ...
  // Function to capture screenshot and create a blob for direct upload
  const captureMapScreenshot = async () => {
    try {
      // Log html2canvas type for debugging
      console.log('html2canvas type:', typeof window.html2canvas);
      
      // First, look for the marked canvas image that was captured when generating the quote
      ...
      if (markedCanvasImg && markedCanvasImg.complete && markedCanvasImg.naturalWidth > 0) {
        console.log("Using pre-captured canvas with markings for quote");
        return markedCanvasImg.src;
      }
      ...
      // Check specific mapContainer information
      console.log('Found map container:', mapContainer.className);
      
      // Use html2canvas to take a screenshot of the map container
      // Import if needed - check if it's a function first
      if (typeof window.html2canvas === 'function') {
        ...
        // Instead of converting to base64, we'll return the canvas element itself
        // This allows the upload function to use the most efficient method (toBlob)
        console.log("Successfully created canvas with html2canvas");
        
        // For backward compatibility, still convert to base64
        // In the future, we could modify this to return the canvas itself
        const base64Image = canvas.toDataURL("image/png");
        console.log("Successfully converted canvas to data URL");
        return base64Image;
      } else {
        console.error('html2canvas is not available as a function');
        return null;
      }
    } catch (error) {
  ...
  const submitToSupabase = async (existingScreenshotUrl?: string) => {
    ...
    const supabaseQuoteData = {
      ...
      quoteData: {
        ...
        linearFeet: mode === 'spaces' ? linearFeet : 0,
    ...
    if (mode === 'area') {
      ...
      return (
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="p-4 bg-background rounded-lg border">
            <p className="text-sm font-medium mb-1">Linear Feet</p>
            <Input
              type="number"
              min="0"
              value={linearFeet || ''}
              onChange={handleNumericChange(setLinearFeet)}
              className="text-xl font-bold text-center"
            />
          </div>
        </div>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        {processedImageId && (
          <div style={{ position: 'absolute', left: '-9999px', top: '-9999px' }}>
            <img 
              id="processed-image" 
              src={typeof processedImageId === 'string' && processedImageId.startsWith('data:') 
                ? processedImageId 
